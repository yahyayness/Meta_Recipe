            ...
            <List sx={{ maxHeight: 350, position: 'relative', overflow: 'auto', }}>
                ...
                </Grid>
                {
                    items.map((item: any)=>{
                        ...
                        return(
                            <Grid container spacing={2} m={1}>
                            <Grid xs={10} className="taste-items" style={{ background: `linear-gradient(90deg, rgba(255, 97, 102, 0.1) ${sValue.value*10}%, #FFF 0%)` }}>
                                <Box className="taste-bg">
                                    <Grid container>
                                        <Grid xs={1} onClick={() => handelDecresing(sValue?.value-.5, item.name,sValue?.id)} className="arrow">
                                        <ArrowLeftOutlinedIcon />
                                        </Grid>
                                        <Grid xs={7} className="taste-name">
                                            {item.name}
                                        </Grid>
                                        <Grid xs={3} className="taste-new-value">
                                            {sValue.value}
                                        </Grid>
                                        <Grid xs={1} onClick={() =>handelIncresing(sValue?.value+.5, item.name,sValue?.id)} className="arrow">
